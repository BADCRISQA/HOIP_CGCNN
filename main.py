...
def main():
    ...
    if args.save_data_path:
        save_training_data(train_loader, args.save_data_path)
    ...
    if args.lr_scheduler == 'multistep':
        ...
        scheduler = CosineAnnealingLR(optimizer, T_max=args.cosine_tmax)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_loss = validate(val_loader, model, criterion, normalizer)
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if test:
        ...
        star_label = '*'
    if args.task == 'regression':
        print(' * MAE {mae_errors.avg:.3f}'
              .format(mae_errors=mae_errors))
        return losses.avg  # Return the average validation loss
    else:
        print(' * Accu {accu.avg:.3f} Prec {prec.avg:.3f} Recall {recall.avg:.3f} F1 {f1.avg:.3f} AUC {auc.avg:.3f}'
              .format(accu=accuracies, prec=precisions, recall=recalls, f1=fscores, auc=auc_scores))
        return losses.avg  # Return the average validation loss
...
def save_training_data(train_loader, save_path):
    import csv
    ...
    with open(save_path, 'w', newline='') as csvfile:
        ...
        # Write header
        csvwriter.writerow(['cif_id'])
        
        for _, _, cif_ids in train_loader:
            for cif_id in cif_ids:
                csvwriter.writerow([cif_id])
    
    print(f"Training data CIF IDs saved to {save_path}")
